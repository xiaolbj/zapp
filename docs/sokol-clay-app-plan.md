...
VirtualList 的嵌套输入遵循按下锁定规则：按下帧使用 VirtualList 与 PrimaryCard 的实际可见矩形交集确定归属，随后一直由列表消费指针位移，直到释放；指针移出列表以及内层到达端点都不会把同一次手势转交给 PrimaryCard。实现不再依赖每帧 `pointerOver`，也不再计算向父级转交的剩余位移，而是在 Clay 更新滚动容器前关闭其通用拖动并手动钳制列表位置，同时恢复外层原位置。鼠标滚轮只要命中列表包装器也由内层独占，到达端点时丢弃剩余增量，但它不建立跨事件捕获，指针移到外层后的下一次滚轮立即由 PrimaryCard 处理。嵌入式滑块使用上一帧真实轨道边界并保持自己的 active ID 到释放。通用交互控件则复用上一帧已按祖先裁剪的语义边界完成按下预判，Button、Slider、TextField、导航/树/表格项等只在 `pointer_down` 为真时用 active ID 阻止父滚动容器接管；释放在控件外会取消点击，遗留 active ID 也不会阻塞后续纯滚轮事件。回归测试覆盖列表滚轮后立即切换外层滚轮、内容区与轨道区滚轮、拖出列表边界、内层端点隔离、普通按钮拖出、外层位置稳定、可见列表项裁切和 scissor 栈平衡。
