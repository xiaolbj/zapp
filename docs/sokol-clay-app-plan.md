...
VirtualList 内容拖动采用内层优先、边界链式传递：内层仍有空间时外层保持不动；内层到达顶部或底部后，仅把被钳制掉的剩余指针位移交给 PrimaryCard，并由外层持续接管到本次手势抬起，避免列表随外层移动出指针后传递中断。列表滚动条命中区域不参与传递，并会扣除 Clay 已经应用给外层的位移，避免重复滚动。回归测试验证内层独立滚动、两端剩余位移传递、传递后指针离开列表仍持续滚动、列表交互后外层仍可精确到达上下边界、可见项裁切，以及全局 scissor 命令数量与嵌套深度平衡。
