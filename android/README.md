...
- “加载所选图片”复用同一分块流，主线程按请求 ID 和绝对偏移组装最多 16 MiB 编码数据，再使用跨平台解码器上传动态 GPU 槽。4 槽缓存以 SHA-256 标识内容并按 LRU 淘汰；命中不重复解码上传，未命中只在新 GPU 对象成功后提交索引和替换 victim。解码仍限制最大维度 4096 与最大 RGBA 数据 64 MiB；加载可取消，失败保留上一张有效纹理。
