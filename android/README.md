...
- “加载所选图片”复用同一分块流，主线程按请求 ID 和绝对偏移组装最多 16 MiB 编码数据，再使用跨平台解码器上传动态 GPU 槽。解码仍限制最大维度 4096 与最大 RGBA 数据 64 MiB；加载可取消，失败保留上一张有效纹理。
...
Image RenderCommand 已在同一模拟器完成运行时验证：资源 Registry 使用与桌面端相同的内存解码桥，在初始化时把首页 PNG 封面和活动页 JPEG 缩略图解码并上传为 Sokol Image/View，共享线性采样 Sampler；cover UV 裁切与圆角纹理网格正确。另将 `/sdcard/Download/zapp-runtime.png` 经系统 DocumentsUI 选为真实 `content://` URI，后台分三块读取 12,266 字节后得到 `128 × 64` 动态纹理；UIAutomator 发现内容描述“所选文件图片预览”的 `android.widget.ImageView`。随后选择文本文件触发“仅支持有效的 PNG 或 JPEG”错误，上一张 ImageView 仍保留，证明 GPU 槽采用成功后替换。全程进程存活，AndroidRuntime、libc 和 crash buffer 均无异常。Gradle 将 `third_party` 纳入 Zig 构建输入，资源解码不依赖 Android 文件路径或 Java Bitmap API。
...
FormField 已在同一模拟器验证：空的必填“应用名称”提交后显示错误边框与中文错误 supporting text，虚拟 EditText 同时暴露必填描述、content-invalid 和 error；外层 PrimaryCard 自动滚动到字段及错误文本。IME 焦点宿主持有焦点时，硬件 Enter 可提交表单，随后一次 Tab 会退出编辑并聚焦“提交表单”按钮；UIAutomator 确认按钮可见且 `focused=true`。这轮同时用运行时截图和语义边界回归测试确认滚动后视觉内容与虚拟节点使用同一坐标。

ChipGroup 的状态筛选示例包含四个虚拟 ToggleButton：默认选中“开发中”和“已完成”，“已归档”禁用。系统点击动作、Enter/Space 会切换受控位掩码；方向键和 Home/End 在可用项间移动并跳过禁用节点。
