...
滚动容器的渲染裁切使用显式 scissor 栈：嵌套裁切先与父区域求交，结束时恢复父区域。由于当前 Clay 版本在可见性剔除时可能保留屏幕外 clip 的结束命令却省略开始命令，项目关闭 Clay 的命令级剔除以保持裁切对完整，并在渲染器中按当前有效裁切区域跳过完全不可见的几何、文字和图片；长列表仍由 VirtualList 控制可见行数量。所有内容超出视口的 Card、ScrollView 和页面容器都会显示可拖拽纵向滚动条；轨道点击可跳转，滑块位置与 Clay 的 `0…-maxScroll` 精确双向映射。首页主卡片内开始的触摸纵向拖动统一归外层卡片，避免手势进入或经过嵌套 VirtualList 时被内层长列表截留；VirtualList 自身仍可使用独立滚动条、鼠标滚轮、键盘和无障碍翻页。
