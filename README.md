...
嵌套滚动采用“按下即锁定、释放才结束”的手势所有权。指针在 VirtualList 可见内容内按下后，整段拖拽始终只改变列表；即使指针移出列表或列表到达端点，也不会转交给外层 Card。滚轮则根据上一帧经过祖先裁剪的滚动语义区域选择面积最小的唯一目标；VirtualList 的内容和嵌入式滚动条包装区都归列表，移到外层后下一次滚轮立即归外层。Clay 仍接收事件以清除惯性，但随后恢复全部滚动快照并只提交显式目标的位置，因此不会因内部 pointer-over 顺序而继续误选列表。普通 Button、Slider、TextField、选择项等交互控件也依据裁剪后的语义边界在按下帧取得所有权，并只在指针仍按下时由控件 active 状态维持，因此从控件内按下再拖出时不会触发父滚动或错误点击。
