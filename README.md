...
嵌套滚动采用“按下即锁定、释放才结束”的手势所有权。指针在 VirtualList 可见内容内按下后，整段拖拽始终只改变列表；即使指针移出列表或列表到达端点，也不会转交给外层 Card。鼠标滚轮在列表包装器内同样由列表严格独占。命中使用列表包装器与 PrimaryCard 的实际矩形交集，嵌入式滑块则使用上一帧真实轨道边界；Clay 更新后恢复明确计算的内外位置。普通 Button、Slider、TextField、选择项等交互控件也依据上一帧经过裁剪的语义边界在按下帧取得所有权，并由控件 active 状态保持到释放，因此从控件内按下再拖出时不会触发父滚动或错误点击。
