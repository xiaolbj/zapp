...
嵌套滚动优先归属 VirtualList；只有列表已经到达输入方向上的真实布局端点时，当前帧未被内层消耗的位移才转交给外层 Card。触摸拖动与鼠标滚轮使用同一规则，且滚轮命中范围覆盖列表内容、嵌入式轨道及两者间隙，避免指针位于轨道时误滚 PrimaryCard。端点计算使用列表固定布局高度，不使用可能被外层裁切缩短的可见包围盒，避免慢拖被提前转交以及部分可见时无法到达首尾。
