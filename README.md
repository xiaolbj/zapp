...
嵌套滚动优先归属 VirtualList；只有列表已经到达输入方向上的真实布局端点时，当前帧未被内层消耗的位移才转交给外层 Card。触摸拖动与鼠标滚轮使用同一规则，且滚轮通过列表包装器与 PrimaryCard 的实际矩形交集命中内容、嵌入式轨道及两者间隙，不依赖真实滚轮事件下可能不稳定的 Clay `pointerOver`。Clay 更新后还会恢复计算好的内外目标位置，避免它再次移动 PrimaryCard。端点计算使用列表固定布局高度，不使用可能被外层裁切缩短的可见包围盒。
